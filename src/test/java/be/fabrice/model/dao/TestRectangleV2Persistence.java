...
import be.fabrice.model.entity.RectangleV2;
...
public class TestRectangleV2Persistence extends AbstractTransactionalTestNGSpringContextTests {
	class RectangleVo {
		Long id;
		double longueur;
		double largeur;
	...
	public void testFindPossibleWithCoherentValues() {
		...
		RectangleV2 r = (RectangleV2) session.get(RectangleV2.class,1000L);
	...
	public void testFindImpossibleWithNonCoherentValues() {
		Session session = sessionFactory.getCurrentSession();
		try {
			session.get(RectangleV2.class,1001L);
			fail();
	...
	public void testTransientRectangleInsertionIsPossible() {
		Session session = sessionFactory.getCurrentSession();
		RectangleV2 rectangle = new RectangleV2(5.5, 3.0);

		session.save(rectangle);

		assertNotNull(rectangle.getId(), "Must have been generated");

		List<RectangleVo> rects = jdbcTemplate.query("select * from RECT2 where ID=?", new RectangleVoMapper(),
				rectangle.getId());
